...
import { normalizeWebhookPayload } from "@/lib/webhook/legacy/normalizer-legacy";
import { computeDedupeKey, extractDedupeParams } from "@/lib/webhook/legacy/deduplicator-legacy";
 ...
 */
export async function handleInlineGet(request: NextRequest, { params }: RouteParams) {
 ...
 */
export async function handleInlinePost(request: NextRequest, { params }: RouteParams) {
  const { webhookKey } = await params;
